...
class LicensePlateRecognizer:
    ...
    def plot_hourly_recognition(self):
        plate_counts = {}
        current = self.linked_list.head

        while current is not None:
            recognized_hour = current.recognized_time.split()[1].split(":")[0]
            if recognized_hour in plate_counts:
                plate_counts[recognized_hour] += 1
            else:
                plate_counts[recognized_hour] = 1
            current = current.next

        hours = list(plate_counts.keys())
        counts = list(plate_counts.values())

        plt.plot(hours, counts)
        plt.xlabel("Цаг")
        plt.ylabel("Машины тоо")
        plt.title("Машины тоо цагаар")
        plt.show()
    ...
    def show_daily_report(self):
        ...
        plot_button = ttk.Button(report_window, text="Үйлчлүүлсэн машины тоо цагаар гаргах", command=self.plot_hourly_recognition)
    ...
    def recognize_plate(self):
        ...
        while True:
            ...
            if max_contour is not None:
                ...
                blurred_roi = cv2.GaussianBlur(plate_roi, (3, 3), 0)
                _, thresholded_roi = cv2.threshold(blurred_roi, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
                opened_roi = cv2.morphologyEx(thresholded_roi, cv2.MORPH_OPEN, np.ones((3, 3), dtype=np.uint8))

                plate_text = pytesseract.image_to_string(opened_roi, lang='mon', config='--psm 7')
