...
class LicensePlateRecognizer:
    ...
    def show_report(self):
        ...
        # Display all recognized plate numbers and recognized times in the search window
    ...
    def search_plate(self, plate_number, result_tree):
        ...
        result_tree.tag_configure("highlight", background="yellow")  # Configure tag for highlighting

        if len(result_tree.get_children()) > 0:
            result_tree.selection_set(result_tree.get_children()[0])  # Select the first matching row
            result_tree.focus(result_tree.get_children()[0])  # Focus on the first matching row
    ...
    def remove_plate(self, plate_number):
        ...
        # Refresh the search report window
        self.show_report()
    ...
    def generate_daily_report(self):
        ...
        # Count the number of cars recognized and calculate the parking duration for each car
        car_count = 0
        report = ""

        current = self.linked_list.head
        while current is not None:
            # Check if the recognized time matches the current date
